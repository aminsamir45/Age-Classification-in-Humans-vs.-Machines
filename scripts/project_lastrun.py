...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/sopsahl/Desktop/9-60-Project/project_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        ...
        languageStyle='LTR',
        depth=0.0);
    continueButton = event.Mouse(win=win)
    x, y = [None, None]
    continueButton.mouseClock = core.Clock()
    
    # --- Initialize components for Routine "trial" ---
    image = visual.ImageStim(
        win=win,
        name='image', 
        image='adversarial_dataset/images/51.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=(0.5, 0.5),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=0.0)
    age_slider = visual.Slider(win=win, name='age_slider',
        startValue=1, size=(1.0, 0.1), pos=(0, -20), units=win.units,
        labels=(0-9, 10-19, 20-29, 30-39, 40-49, 50-59), ticks=(1, 2, 3, 4,5, 6), granularity=0.0,
    ...
    text_3 = visual.TextStim(win=win, name='text_3',
        text='Thanks for participating in our 9.60 Project!!\n\nClick anywhere on the screen to terminate :)\n\nPlease email sopsahl@mit.edu if you have any feedback (positive or negative).',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    continueRoutine = continueRoutine and not (continueButton.clicked)
    ...
    instructionsComponents = [text, continueButton]
    ...
    while continueRoutine and routineTimer.getTime() < 30.0:
        ...
        if continueButton.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # add timestamp to datafile
            thisExp.addData('continueButton.started', t)
            # update status
            continueButton.status = STARTED
            prevButtonState = continueButton.getPressed()  # if button is down already this ISN'T a new click
        
        # if continueButton is stopping this frame...
        if continueButton.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > continueButton.tStartRefresh + 30.0-frameTolerance:
                # keep track of stop time/frame for later
                continueButton.tStop = t  # not accounting for scr refresh
                continueButton.tStopRefresh = tThisFlipGlobal  # on global time
                continueButton.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('continueButton.stopped', t)
                # update status
                continueButton.status = FINISHED
        if continueButton.status == STARTED:  # only update if started and not finished!
            buttons = continueButton.getPressed()
            if buttons != prevButtonState:  # button state changed?
                prevButtonState = buttons
                if sum(buttons) > 0:  # state changed to a new click
                    x, y = continueButton.getPos()
                    continueButton.x.append(x)
                    continueButton.y.append(y)
                    buttons = continueButton.getPressed()
                    continueButton.leftButton.append(buttons[0])
                    continueButton.midButton.append(buttons[1])
                    continueButton.rightButton.append(buttons[2])
                    continueButton.time.append(continueButton.mouseClock.getTime())
    ...
    thisExp.addData('continueButton.x', continueButton.x)
    thisExp.addData('continueButton.y', continueButton.y)
    thisExp.addData('continueButton.leftButton', continueButton.leftButton)
    thisExp.addData('continueButton.midButton', continueButton.midButton)
    thisExp.addData('continueButton.rightButton', continueButton.rightButton)
    thisExp.addData('continueButton.time', continueButton.time)
    ...
    for thisTrial in trials:
        ...
        # skip this Routine if its 'Skip if' condition is True
        continueRoutine = continueRoutine and not (enter)
        age_slider.reset()
        key_resp.keys = []
        key_resp.rt = []
        _key_resp_allKeys = []
        # keep track of which components have finished
        trialComponents = [image, age_slider, key_resp, text_2]
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # if age_slider is stopping this frame...
            if age_slider.status == STARTED:
                if bool(10.0):
                    # keep track of stop time/frame for later
                    age_slider.tStop = t  # not accounting for scr refresh
                    age_slider.tStopRefresh = tThisFlipGlobal  # on global time
                    age_slider.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'age_slider.stopped')
                    # update status
                    age_slider.status = FINISHED
            ...
            if key_resp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > key_resp.tStartRefresh + 10.0-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        trials.addData('age_slider.response', age_slider.getRating())
        trials.addData('age_slider.rt', age_slider.getRT())
        # check responses
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('terminate.started', globalClock.getTime(format='float'))
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (mouse.clicked)
    ...
    gotValidClick = False  # until a click is received
    # keep track of which components have finished
    terminateComponents = [text_3, mouse]
    for thisComponent in terminateComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine and routineTimer.getTime() < 10.0:
        ...
        if text_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_3.tStartRefresh + 10.0-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in terminateComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('terminate.stopped', globalClock.getTime(format='float'))
    # store data for thisExp (ExperimentHandler)
